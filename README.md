...
- **轮询到的新推按普通图文逐条发**：一条推文一条消息（正文 + 配图），最旧的先发，
  聊天里从上往下读就是时间顺序；视频单独成条。
- **`/tw_test` 预览按合并转发发**：一条聊天记录装下预览的多条推文，
  **每条推文一个节点、最新的排在最上面**。两种格式各有一个开关，可自由对调。
...
| `display.use_forward` | `true` | 合并转发总开关（关掉则一律普通图文） |
| `display.forward_for_poll` | **`false`** | 自己轮询到的新推是否用合并转发（默认普通图文逐条发） |
| `display.forward_for_test` | **`true`** | `/tw_test` 预览是否用合并转发 |
| `display.batch_forward` | `true` | 合并转发时多条推文是否装进同一条聊天记录 |
...
| `[display]` | 合并转发总开关、**轮询/预览各自的格式开关**、批量打包、聊天记录体积上限、视频进节点、昵称、作者行、分隔线、链接与统计、正文截断 |
...
- **两种投递格式**（由两个开关分别控制，`display.use_forward` 是总开关）：

  | 触发方式 | 默认格式 | 开关 |
  | --- | --- | --- |
  | 自己轮询到的新推文（含 `/tw_check`、订阅时的"推最新一条"） | **普通图文**：一条推文一条消息（正文 + 配图），**最旧的先发**，视频单独成条 | `display.forward_for_poll`（默认 `false`） |
  | `/tw_test` 预览 | **合并转发**：一条聊天记录装多条推文，**每条推文一个节点、最新的排在最上面** | `display.forward_for_test`（默认 `true`） |

  两个开关可以对调（比如想要轮询也发聊天记录，就把 `forward_for_poll` 设成 `true`）。
- **合并转发时的视频推文占两个相邻节点**：QQ 协议规定「video 必须是消息里唯一的元素」
  （原文 `message element "video" must be the only segment in a message`），
  所以视频推文先是正文节点、紧跟一个只有视频的节点，整体仍在同一条聊天记录里。
  普通图文格式下不涉及节点，视频直接单独成条。
  > 更深一层的「节点里再套聊天记录」不被适配器支持（会打
  > `SnowLuma 跳过无法转换的出站消息段: type=forward`），整条消息会发送失败。
- **自动拆分**：合并转发时图片总量超过 `display.batch_max_mb` 或节点数超限，自动拆成多条聊天记录。
...
正文里的多余空行会被收敛。合并转发时每条推文一个节点、**最新的排在最上面**；
普通图文时每条推文各发一条消息、**最旧的先发**。
